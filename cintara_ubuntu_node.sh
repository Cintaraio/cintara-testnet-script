...
# Set client config
  ...
  # Allocate genesis accounts (cosmos formatted addresses)
    ...
    jq '.app_state["mint"]["params"]["mint_denom"]="cint"' >"$TMP_GENESIS" "$GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state["gov"]["params"]["expedited_voting_period"]="1200s"' >"$TMP_GENESIS" "$GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
    ...
    sed -i 's/pruning-keep-recent = "0".tmp-cintarad/pruning-keep-recent = "100000"/g' "$APP_TOML"
...
Group=$(whoami)
Type=simple
ExecStart=/home/ubuntu/go/bin/cosmovisor run start --home $HOMEDIR
Restart=always
RestartSec=3
LimitNOFILE=4096
...
[Install]
WantedBy=multi-user.target'> /etc/systemd/system/cintarachain.service"
